...
def welcome():
    ...
    if session.get("is_logged_in", False):
        ...
        return render_template("welcome.html", email=session["email"], name=session["name"], report_data=report_data, has_report_data=True, graph=graph, roomTypes={"Suite Room": -1, "Family Room": 0, "Deluxe Room": 1, "Classic Room": 1, "Superior Room": 1, "Luxury Room": 1, "Suite Rooms": 1})
    else:
        # If user is not logged in, redirect to login page
